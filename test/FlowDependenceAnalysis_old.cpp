//===- FlowDependenceAnalysis.cpp - Mem Deps Implementation -------------===//
//
...
char FlowDependenceAnalysis::ID = 0;

// Register this pass...
// INITIALIZE_PASS_BEGIN(FlowDependenceAnalysis, "memdep",
//                "Memory Dependence Analysis", false, true)
// INITIALIZE_AG_DEPENDENCY(AliasAnalysis)
// INITIALIZE_PASS_END(FlowDependenceAnalysis, "memdep",
//                      "Memory Dependence Analysis", false, true)

/* if Analysis Pass, the 4th arg should be true*/
static RegisterPass<FlowDependenceAnalysis>
    FDA("fda", "Flow  Dependence Analysis", false, true);

FlowDependenceAnalysis::FlowDependenceAnalysis()
    : FunctionPass(ID), PredCache() {}
FlowDependenceAnalysis::~FlowDependenceAnalysis() {}

/// Clean up memory in between runs
void FlowDependenceAnalysis::releaseMemory() {
  LocalDeps.clear();
  NonLocalDeps.clear();
...
void FlowDependenceAnalysis::getAnalysisUsage(AnalysisUsage &AU) const {
  ...
  // Why Transitive?If MDA does addRequiredTransitive<AA> instead, AA
...
static ModRefInfo GetLocation(const Instruction *Inst, MemoryLocation &Loc,
  ...
  if (const LoadInst *LI = dyn_cast<LoadInst>(Inst)) {
    /// Zhiyuan: TODO: what is "UNORDERED"
    if (LI->isUnordered()) {
      Loc = MemoryLocation::get(LI);
      return ModRefInfo::Ref;
    }
    /// Zhiyuan: TODO: what is "MONOTONIC"
  ...
  if (const IntrinsicInst *II = dyn_cast<IntrinsicInst>(Inst))
    ...
    case Intrinsic::invariant_start:
      // TODO:
      //   Loc = MemoryLocation(II->getArgOperand(1),
    ...
    case Intrinsic::invariant_end:
      // TODO:
      //   Loc = MemoryLocation(II->getArgOperand(2),
...
bool FlowDependenceAnalysis::runOnFunction(Function &F) {
  ...
  setTLI(getAnalysis<TargetLibraryInfoWrapperPass>().getTLI());
  ...
  errs() << "FlowDependenceAnalysis::runOnFunction Local Deps: "
         << LocalDeps.size() << "\n";
...
FlowDepResult FlowDependenceAnalysis::getCallSiteDependencyFrom(
  ...
  while (ScanIt != BB->begin()) {
    ...
    --Limit;
    if (!Limit)
      return FlowDepResult::getUnknown();

    Instruction *Inst = dyn_cast<Instruction>(--ScanIt);

    // If this inst is a memory op, get the pointer it accessed
    MemoryLocation Loc;
    ModRefInfo MR = GetLocation(Inst, Loc, AA, TLI);
    if (Loc.Ptr) {
      // A simple instruction.
      if (AA->getModRefInfo(CS, Loc) != ModRefInfo::NoModRef)
        return FlowDepResult::getClobber(Inst);
      continue;
    }

    if (CallSite *InstCS = cast<CallSite>(Inst)) {
      ...
      case ModRefInfo::NoModRef:
        ...
        if (isReadOnlyCall && !(MR & ModRefInfo::Mod) &&
            CS.getInstruction()->isIdenticalToWhenDefined(Inst))
          return FlowDepResult::getDef(Inst);
  ...
  if (BB != &BB->getParent()->getEntryBlock())
    return FlowDepResult::getNonLocal();
  return FlowDepResult::getNonFuncLocal();
}
...
static bool isLoadLoadClobberIfExtendedToFullWidth(const MemoryLocation &MemLoc,
  ...
  unsigned Size = FlowDependenceAnalysis::getLoadLoadClobberFullWidthSize(
      MemLocBase, MemLocOffs, MemLoc.Size, LI, *DL);
  return Size != 0;
}
...
unsigned FlowDependenceAnalysis::getLoadLoadClobberFullWidthSize(
    const Value *MemLocBase, int64_t MemLocOffs, unsigned MemLocSize,
    const LoadInst *LI, const DataLayout &DL) {
  ...
  if (LI->getParent()->getParent()->getAttributes().hasAttribute(
          AttributeSet::FunctionIndex, Attribute::SanitizeThread))
    return 0;
  ...
  while (1) {
    ...
    if (LIOffs + NewLoadByteSize > MemLocEnd &&
        LI->getParent()->getParent()->getAttributes().hasAttribute(
            AttributeSet::FunctionIndex, Attribute::SanitizeAddress))
...
FlowDepResult FlowDependenceAnalysis::getPointerDependencyFrom(
  ...
  /// Walk backwards starting from the instruction "ScanIt"
  while (ScanIt != BB->begin()) {
    Instruction *Inst = dyn_cast<Instruction>(--ScanIt);
    ...
    if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(Inst)) {
      ...
      if (II->getIntrinsicID() == Intrinsic::lifetime_start) {
        ...
        if (AA->isMustAlias(MemoryLocation(II->getArgOperand(1)), MemLoc))
          return FlowDepResult::getDef(II);
        continue;
      }
    }
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(Inst)) {
      ...
      /// Zhiyuan: ? Is this the reason why there are too many clobbers in the
      /// ToyPointer code? Zhiyuan ? what is unordered?
      if (!LI->isUnordered())
        return FlowDepResult::getClobber(LI);

      MemoryLocation LoadLoc = AA->getLocation(LI);
      ...
      if (isLoad) {
        ...
        if (R == MustAlias) {
          /// FIXME: Zhiyuan: there is no data dependence between QueryInst and
          /// Inst according to our definition.
          /** it is a read after read (RAR).
           *  (1) a = *p;
           *  (2) q = p;
           *  (3) b = *p;
           *  (1) and (3) are both Load instruction, they do not have "Def"
           * relation inside.
           */
          return FlowDepResult::getDef(Inst);
        }
      ...
      /// Zhiyuan: Since we have already handled LOAD instruction, now we handle
      /// STORE instruction. Store after Load (WAR) => Anti-dependency
      ...
      /// Zhiyuan: FIXME: not what we are looking for (WAR)
      /**
       *  b = *p;
       *  *p = a;
       *
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(Inst)) {
      /// Zhiyuan: in this case, we handle XAW, which includes RAW and WAW.
      // Atomic stores have complications involved.
      // FIXME: This is overly conservative.
      if (!SI->isUnordered())
        return FlowDepResult::getClobber(SI);

      // If alias analysis can tell that this store is guaranteed to not modify
      // the query pointer, ignore it.  Use getModRefInfo to handle cases where
      // the query pointer points to constant memory etc.
      if (AA->getModRefInfo(SI, MemLoc) == ModRefInfo::NoModRef)
        continue;

      // Ok, this store might clobber the query pointer.  Check to see if it is
      // a must alias: in this case, we want to return this as a def.
      MemoryLocation StoreLoc = AA->getLocation(SI);
      ...
      if (isInvariantLoad)
        continue;
      /// ? Zhiyuan: this is for AliasAnalysis::MayAlias?
      return FlowDepResult::getClobber(Inst);
    }
    ...
    if (isa<AllocaInst>(Inst) || isNoAliasFn(Inst, TLI)) {
      ...
      if (isa<AllocaInst>(Inst) || isMallocLikeFn(Inst, TLI) ||
...
/// depends.
FlowDepResult FlowDependenceAnalysis::getDependency(Instruction *QueryInst) {
  Instruction *ScanPos = QueryInst;

  // Check for a cached result
  FlowDepResult &LocalCache = LocalDeps[QueryInst];

  // If the cached entry is non-dirty, just return it.  Note that this depends
  // on FlowDepResult's default constructing to 'dirty'.
  /// Zhiyuan: yes, if QueryInst does not exist inside LocalDeps,
  /// LocalDeps[QueryInst] will call the constructor where dirty is set to be
  /// true.
  if (!LocalCache.isDirty())
    return LocalCache;

  if (llvm::DebugFlag) {
    errs() << "LocalCache is dirty.\n";
  }
  ...
  /**
   * We get the Basic Block for this instruction.
   */
  // Do the scan.
  if (BasicBlock::iterator(QueryInst) == QueryParent->begin()) {
    //    errs() << "**** BasicBlock::iterator(QueryInst) ==
    //    QueryParent->begin()\n"; errs() << "****
    //    BasicBlock::iterator(QueryInst) =" << *BasicBlock::iterator(QueryInst)
    //    << "\n"; errs() << "**** QueryParent->begin()            =" <<
    //    *QueryParent->begin() << "\n";
    /// Zhiyuan: if this is the first instruction of the basic block.
    // No dependence found.  If this is the entry block of the function, it is
    // unknown, otherwise it is non-local.
    //    errs() << "**** QueryParent                                 =" <<
    //    *QueryParent << "\n"; errs() << "****
    //    QueryParent->getParent()->getEntryBlock()   =" <<
    //    QueryParent->getParent()->getEntryBlock() << "\n";
    if (QueryParent != &QueryParent->getParent()->getEntryBlock()) {
      /// Zhiyuan: if it is not the entry block of the function.
      LocalCache = FlowDepResult::getNonLocal();
    } else {
      /// Zhiyuan: if it is the entry block of the function.
      LocalCache = FlowDepResult::getNonFuncLocal();
    }
  } else {

    //    errs() << "**** BasicBlock::iterator(QueryInst) =" <<
    //    *BasicBlock::iterator(QueryInst) << "\n"; errs() << "****
    //    QueryParent->begin()            =" << *QueryParent->begin() << "\n";
    /// Zhiyuan: if this is not the first instruction of the basic block.
    MemoryLocation MemLoc;
    /// Zhiyuan: use AliasAnalysis's ModRefResult to check the Mod/Ref info
    /// between the instruction and the memory it accesses.
    ModRefResult MR = GetLocation(QueryInst, MemLoc, AA, TLI);
    /// Zhiyuan: if the memory location is not null (that means the instruction
    /// actually access memory.)
    if (MemLoc.Ptr) {
      // If we can do a pointer scan, make it happen.
      /// AliasAnalysis::Mod == 0x10, when MR == NoModRef or Ref, isLoad == 1,
      /// when MR == Mod or ModRef, isLoad == 0
      bool isLoad = !(MR & ModRefInfo::Mod);
      /// ? what is IntrinsicInst? class IntrinsicInst : public CallInst
      ...
      LocalCache =
          getCallSiteDependencyFrom(QueryCS, isReadOnly, ScanPos, QueryParent);
    } else
      // Non-memory instruction.
      LocalCache = FlowDepResult::getUnknown();
  }
...
static void AssertSorted(FlowDependenceAnalysis::NonLocalDepInfo &Cache,
                         int Count = -1) {
  if (Count == -1)
    Count = Cache.size();
  if (Count == 0)
...
/// that.
const FlowDependenceAnalysis::NonLocalDepInfo &
FlowDependenceAnalysis::getNonLocalCallDependency(CallSite QueryCS) {
  ...
  while (!DirtyBlocks.empty()) {
    ...
    if (ScanPos != DirtyBB->begin()) {
      ...
      Dep = FlowDepResult::getNonLocal();
    } else {
      Dep = FlowDepResult::getNonFuncLocal();
    }
...
/// own block.
///
void FlowDependenceAnalysis::getNonLocalPointerDependency(
    const MemoryLocation &Loc, bool isLoad, BasicBlock *FromBB,
    SmallVectorImpl<NonLocalDepResult> &Result) {
  assert(Loc.Ptr->getType()->isPointerTy() &&
         "Can't get pointer deps of a non-pointer!");
  Result.clear();

  errs() << "now in getNonLocalPointerDependency!" << '\n';
  ...
  if (!getNonLocalPointerDepFromBB(Address, Loc, isLoad, FromBB, Result,
                                   Visited, true))
    return;
  // Result.clear();
  Result.push_back(NonLocalDepResult(FromBB, FlowDepResult::getUnknown(),
...
FlowDepResult FlowDependenceAnalysis::GetNonLocalInfoForBlock(
  ...
  FlowDepResult Dep = getPointerDependencyFrom(Loc, isLoad, ScanPos, BB);
...
static void
SortNonLocalDepInfoCache(FlowDependenceAnalysis::NonLocalDepInfo &Cache,
  ...
  case 2: {
    ...
    Cache.pop_back();
    FlowDependenceAnalysis::NonLocalDepInfo::iterator Entry =
...
bool FlowDependenceAnalysis::getNonLocalPointerDepFromBB(
    const PHITransAddr &Pointer, const MemoryLocation &Loc, bool isLoad,
    BasicBlock *StartBB, SmallVectorImpl<NonLocalDepResult> &Result,
  ...
  if (!Pair.second) {
    if (CacheInfo->Size < Loc.Size) {
      ...
      return getNonLocalPointerDepFromBB(
          Pointer, Loc.getWithNewSize(CacheInfo->Size), isLoad, StartBB, Result,
  ...
  if (CacheInfo->Pair == BBSkipFirstBlockPair(StartBB, SkipFirstBlock)) {
    ...
    for (NonLocalDepInfo::iterator I = Cache->begin(), E = Cache->end(); I != E;
      ...
      if (!DT) {
        Result.push_back(
            NonLocalDepResult(I->getBB(), FlowDepResult::getUnknown(), Addr));
  ...
  while (!Worklist.empty()) {
    ...
    if (!SkipFirstBlock) {
      ...
      DEBUG(AssertSorted(*Cache, NumSortedEntries));
      FlowDepResult Dep =
          GetNonLocalInfoForBlock(Loc, isLoad, BB, Cache, NumSortedEntries);

      // If we got a Def or Clobber, add this to the list of results.
      if (!Dep.isNonLocal()) {
        if (!DT) {
          Result.push_back(NonLocalDepResult(BB, FlowDepResult::getUnknown(),
                                             Pointer.getAddr()));
          continue;
    ...
    if (!Pointer.NeedsPHITranslationFromBlock(BB)) {
      ...
      for (BasicBlock **PI = PredCache->GetPreds(BB); *PI; ++PI) {
        ...
        std::pair<DenseMap<BasicBlock *, Value *>::iterator, bool> InsertRes =
            Visited.insert(std::make_pair(*PI, Pointer.getAddr()));
        if (InsertRes.second) {
    ...
    for (unsigned i = 0, n = PredList.size(); i < n; ++i) {
      ...
      if (!CanTranslate || getNonLocalPointerDepFromBB(
                               PredPointer, Loc.getWithNewPtr(PredPtrVal),
                               isLoad, Pred, Result, Visited)) {
        // Add the entry to the Result list.
        NonLocalDepResult Entry(Pred, FlowDepResult::getUnknown(), PredPtrVal);
  ...
  PredTranslationFailure:
    ...
    for (NonLocalDepInfo::reverse_iterator I = Cache->rbegin();; ++I) {
      ...
      I->setResult(FlowDepResult::getUnknown());
      Result.push_back(
...
void FlowDependenceAnalysis::RemoveCachedNonLocalPointerDependencies(
    ValueIsLoadPair P) {
...
void FlowDependenceAnalysis::invalidateCachedPointerInfo(Value *Ptr) {
...
/// critical edges.
void FlowDependenceAnalysis::invalidateCachedPredecessors() {
  PredCache->clear();
}
...
void FlowDependenceAnalysis::removeInstruction(Instruction *RemInst) {
  ...
  FlowDepResult NewDirtyVal;
  if (!RemInst->isTerminator()) {
    // NewDirtyVal = getDirty(++BasicBlock::iterator(RemInst));
    RemInst = RemInst->getNextNode();
    NewDirtyVal = FlowDepResult::getDirty(RemInst);
  }
...
void FlowDependenceAnalysis::verifyRemoved(Instruction *D) const {
...
// static RegisterPass<FlowDependenceAnalysis> FDA("fda",
// "FlowDependenceAnalysis calling", false, false);

/*
  for (Function::iterator BB = F.begin(), BE = F.end(); BB != BE; ++BB) {
  errs() << "Function::iterrator :" << BB->getName() << '\n';
  for (BasicBlock::iterator I = BB->begin(), IE = BB->end(); I != IE; ++I) {
  errs() << "BasicBlock::iterator :" << *I << "\n";
  FlowDepResult res = getDependency(I);
  //errs() << "Local Deps: " << LocalDeps.size() << "\n";

  Instruction *pInstruction = dyn_cast<Instruction>(&*I);


  if(!isa<LoadInst>(pInstruction) && !isa<StoreInst>(pInstruction))
  continue;

  if (nullptr == res.getInst()) {
  errs() << "I has no dependence" << '\n';
  }
  else
  errs() << "getDependency res = " << *res.getInst() << '\n';

  if (!res.getInst()) {
  errs() << "!res.getInst()" << '\n';
  if (res.isNonLocal()) {
  errs() << "isNonLocal: " << *I << "\n";
  Function::iterator BB1 = BB;

  for (Function::iterator BE1 = F.begin(); BB1 != BE1; --BB1) {

  SmallVector<NonLocalDepResult, 20> Result;
  AliasAnalysis::Location MemLoc;
  AliasAnalysis::ModRefResult MR = GetLocation(I, MemLoc, AA);
  errs() << "MemLoc: " << MemLoc.Ptr << "\n";

  if (MemLoc.Ptr) {
  /// AliasAnalysis::Mod == 0x10, when MR == NoModRef or Ref, isLoad == 1, when
  MR == Mod or ModRef, isLoad == 0 bool isLoad = !(MR & AliasAnalysis::Mod);
  getNonLocalPointerDependency(MemLoc, isLoad, BB1, Result);
  errs() << "Result.size(): " << Result.size() << "\n";
  for (SmallVector<NonLocalDepResult, 20>::iterator it = Result.begin(); it !=
  Result.end(); it++) { NonLocalDepResult nld = *it; errs() << "Inst: " <<
  nld.getResult().getInst() << "\n";
  }
  }//end if MemLoc.Ptr
  }//end for
  ...
  //errs() << "IsDef: [" << *I << "] depends on [" << *res.getInst() << "]\n";
  } else if (res.isClobber()) {
  // There might be a dependency with res.getInst(). Let's be
  // conservative.
  //errs() << "isClobber: [" << *I << "] depends on [" << *res.getInst() <<
  "]\n";
  }
