//===- llvm/Analysis/MemoryDependenceAnalysisWrapper.h - Memory Deps  --*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the MemoryDependenceAnalysisWrapper analysis pass.
//
//===----------------------------------------------------------------------===//
...
namespace llvm {
  ...
  /// FlowDepResult - A memory dependence query can return one of three different
  ...
  class FlowDepResult {
    enum DepType {
      ...
      /// clobbers the desired value.  The pointer member of the FlowDepResult
      ...
      /// the FlowDepResult pair holds the instruction that defines the memory.
      /// Cases of interest:
    ...
    PairTy Value;
    explicit FlowDepResult(PairTy V) : Value(V) {}
  public:
    FlowDepResult() : Value(nullptr, Invalid) {}

    /// get methods: These are static ctor methods for creating various
    /// FlowDepResult kinds.
    static FlowDepResult getDef(Instruction *Inst) {
      assert(Inst && "Def requires inst");
      return FlowDepResult(PairTy(Inst, Def));
    }
    static FlowDepResult getClobber(Instruction *Inst) {
      assert(Inst && "Clobber requires inst");
      return FlowDepResult(PairTy(Inst, Clobber));
    }
    static FlowDepResult getNonLocal() {
      return FlowDepResult(
        PairTy(reinterpret_cast<Instruction*>(NonLocal), Other));
    }
    static FlowDepResult getNonFuncLocal() {
      return FlowDepResult(
        PairTy(reinterpret_cast<Instruction*>(NonFuncLocal), Other));
    }
    static FlowDepResult getUnknown() {
      return FlowDepResult(
        PairTy(reinterpret_cast<Instruction*>(Unknown), Other));
    }

    static FlowDepResult getDirty(Instruction *Inst) {
      return FlowDepResult(PairTy(Inst, Invalid));
    }

    /// isClobber - Return true if this FlowDepResult represents a query that is
    /// an instruction clobber dependency.
    bool isClobber() const { return Value.getInt() == Clobber; }

    /// isDef - Return true if this FlowDepResult represents a query that is
    /// an instruction definition dependency.
    bool isDef() const { return Value.getInt() == Def; }

    /// isNonLocal - Return true if this FlowDepResult represents a query that
    ...
    /// isNonFuncLocal - Return true if this FlowDepResult represents a query
    ...
    /// isUnknown - Return true if this FlowDepResult represents a query which
    ...
    bool operator==(const FlowDepResult &M) const { return Value == M.Value; }
    bool operator!=(const FlowDepResult &M) const { return Value != M.Value; }
    bool operator<(const FlowDepResult &M) const { return Value < M.Value; }
    bool operator>(const FlowDepResult &M) const { return Value > M.Value; }
  private:
    friend class FlowDependenceAnalysis;
    ...
    ///
    /// In a default-constructed FlowDepResult object, the type will be Dirty
    /// and the instruction pointer will be null.
    ///

    /// isDirty - Return true if this is a FlowDepResult in its dirty/invalid.
    /// state.
  ...
  /// each BasicBlock (the BB entry) it keeps a FlowDepResult.
  class NonLocalDepEntry {
    BasicBlock *BB;
    FlowDepResult Result;
  public:
    NonLocalDepEntry(BasicBlock *bb, FlowDepResult result)
      : BB(bb), Result(result) {}
    ...
    void setResult(const FlowDepResult &R) { Result = R; }

    const FlowDepResult &getResult() const { return Result; }
  ...
  /// For each BasicBlock (the BB entry) it keeps a FlowDepResult and the
  /// (potentially phi translated) address that was live in the block.
  class NonLocalDepResult {
    NonLocalDepEntry Entry;
    Value *Address;
  public:
    NonLocalDepResult(BasicBlock *bb, FlowDepResult result, Value *address)
      : Entry(bb, result), Address(address) {}

    // BB is the sort key, it can't be changed.
    BasicBlock *getBB() const { return Entry.getBB(); }

    void setResult(const FlowDepResult &R, Value *Addr) {
      Entry.setResult(R);
      Address = Addr;
    }

    const FlowDepResult &getResult() const { return Entry.getResult(); }
  ...
  /// FlowDependenceAnalysis - This is an analysis that determines, for a
  ...
  ///
  class FlowDependenceAnalysis : public FunctionPass {
    // A map from instructions to their dependency.
    typedef DenseMap<Instruction*, FlowDepResult> LocalDepMapType;
    LocalDepMapType LocalDeps;

  public:
    typedef std::vector<NonLocalDepEntry> NonLocalDepInfo;

    const TargetLibraryInfo& getTLI() {
      return TLI;
    }
  ...
  private:
    ...
    struct NonLocalPointerInfo {
      ...
      NonLocalPointerInfo()
        : Size(MemoryLocation::UnknownSize), TBAATag(nullptr) {}
    };
  ...
  public:
    ...
    FlowDepResult getDependency(Instruction *QueryInst);
    ...
    void getNonLocalPointerDependency(const MemoryLocation &Loc,
                                      bool isLoad, BasicBlock *BB,
                                    SmallVectorImpl<NonLocalDepResult> &Result);
    

    std::vector<Instruction*> getDependencyInFunction(Function &F, Instruction* pLoadInst);
    ...
    ///
    FlowDepResult getPointerDependencyFrom(const MemoryLocation &Loc,
                                          bool isLoad,
  ...
  private:
    FlowDepResult getCallSiteDependencyFrom(CallSite C, bool isReadOnlyCall,
                                           BasicBlock::iterator ScanIt,
                                           BasicBlock *BB);
    ...
    FlowDepResult GetNonLocalInfoForBlock(const MemoryLocation &Loc,
                                         bool isLoad, BasicBlock *BB,
